...
export default function ProjectList({ projects, selectedId, onCreate, onDelete, onSelect, onSignOut }: Props) {
  ...
  const colorHex = (id: string) => CALENDAR_COLORS.find(c => c.id === id)?.hex ?? '#7986cb'
    ...
    <aside style={styles.sidebar}>
      ...
      <div style={styles.projectList}>
        {projects.map(p => (
          <div
            key={p.id}
            style={{ ...styles.projectRow, background: selectedId === p.id ? '#1e3a5f' : 'transparent' }}
            onClick={() => onSelect(p.id)}
          >
            <span style={{ ...styles.dot, background: colorHex(p.color) }} />
...
const styles: Record<string, React.CSSProperties> = {
  ...
  deleteBtn: { background: 'none', border: 'none', color: '#475569', cursor: 'pointer', fontSize: 18, lineHeight: 1, padding: '0 2px', display: 'none' },
